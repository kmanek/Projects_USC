...
ifstream file( "patient1.txt", ios::binary ) ;
if(!file)
    {
...
//done reading of file patient1.txt
 //reading file patiient1insurance.txt
    ifstream file2( "patient1insurance.txt", ios::binary ) ;
if(!file2)
    {
perror("Error opening file");
return(-1);
...
while(!file2.eof())
    ...
    //done reading of file patiient1insurance.txt
    string f(st[1]);
int sockfd, numbytes;
...
int res=recv(sockfd,b3,1024,0);//dummy receive as sending authenticate,username,password separately
    ...
    cout<<"Phase 1 : Patient 1 has TCP port number "<<ntohs(sin.sin_port)<<" and IP address "<<inet_ntoa(sin.sin_addr)<<endl;
    cout<<"Phase 1 : Authentication request from Patient 1 with username "<<st[0]<<" and password "<<f<<" has been sent to Health Center Server."<<endl;
    cout<<"Phase 1 : Patient 1 authentication result: "<<b2<<"."<<endl;


    string b2_s(b2);
    if (b2_s=="failure")//exit phase 1 if authentication fails
        return 0;
    cout<<"Phase 1 : End of Phase 1 for Patient 1."<<endl;
        //////phase 1 ends
    ...
    cout<<"Phase 2 : The following appointments are available for Patient1:"<<endl<<avail<<endl;
    ...
    {
        cout<<"Phase 2 : The requested appointment from Patient1 is not available.Exiting..."<<endl;
        return 0;
    }
    else{
            int start=phrase_s.find(" ");
            string doc=phrase_s.substr(0,start);
            string port=phrase_s.substr(start+1,'\r');
            cout<<"Phase 2 : The requested appointment is available and reserved to Patient1.The assigned doctor port number is "<<port<<"."<<endl;
...
sendto(sockfd,st[0].c_str(), strlen(st[0].c_str()), 0,p->ai_addr, p->ai_addrlen);
int z;
z=atoi(buf1);
...
s, sizeof s);
//////////////////////
cout<<"Phase 3 : Patient 1 has a dynamic UDP port number "<<ntohs(sin.sin_port)<<" and IP address "<<s<<endl;
...
cout<<"Phase 3 : Patient 1 receives "<<z<<"$ estimation cost from doctor with port number "<<port<<" and name "<<doc<<"."<<endl;
cout<<"Phase 3 : End of Phase 3 for Patient1."<<endl;
